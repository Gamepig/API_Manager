...
class MainApp(QMainWindow):
    ...
    def __init__(self):
        ...
        self.action_worker.finished.connect(self.load_api_data_finished) # 動作完成後也刷新數據
        # 連接自定義信號到 worker 的任務方法
    ...
    def load_api_data(self):
        ...
        """
        print("載入 API 數據... (開始)")
        self.loading_overlay.set_message("載入 API 數據中...")
        self.loading_overlay.show_overlay()

        # 發射信號以在 worker 線程中開始數據載入
    ...
    def display_api_details(self, item: QTreeWidgetItem):
        ...
        """
        # Check if the clicked item is an API (has UserRole data)
        ...
        if api_data and api_data.get("type") != "project": # 確保是 API 項目而不是專案項目
            ...
            memory_usage = api_data.get("memory", 0) / (1024 * 1024) # 轉換為 MB
    ...
    def setup_data_refresh_timer(self):
        ...
        self.timer.setInterval(1000) # 每 1 秒刷新一次
        ...
        if item:
            ...
            if api_data and api_data.get("type") == "project":
                ...
                if api_id is not None:
                    ...
                    start_action.triggered.connect(lambda: pm2_manager.start_api(api_id))
                    restart_action.triggered.connect(lambda: pm2_manager.restart_api(api_id))
                    stop_action.triggered.connect(lambda: pm2_manager.stop_api(api_id))
